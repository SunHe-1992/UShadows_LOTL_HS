...
void AUS_Character::Interact_Server_Implementation()
{
	if (InteractableActor)
	{
		...
		GEngine->AddOnScreenDebugMessage(3, 5.f, FColor::Red, TEXT("Interact_Server" + InteractableActor->GetName()));
	}
	else {
...
void AUS_Character::Tick(float DeltaTime)
{
	...
	if (IsHit && HitResult.GetActor()) {
		auto actName = HitResult.GetActor()->GetName();
		GEngine->AddOnScreenDebugMessage(3, 5.f, FColor::Red, TEXT("IsHit actor" + actName));
	}
	...
	{
		DrawDebugSphere(GetWorld(), HitResult.ImpactPoint, SphereRadius, 12, FColor::Green, false, 1.f);
		InteractableActor = HitResult.GetActor();
		GEngine->AddOnScreenDebugMessage(3, 5.f, FColor::Red, TEXT("found InteractableActor" + InteractableActor->GetName()));
	}
	else
	{
